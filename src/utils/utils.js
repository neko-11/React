...
export const AsyncPost= (name ,body ,type ,successCallback,failCallback) =>{
    // if(process.env.NODE_ENV === 'production'){
    if (sessionStorage.getItem("userName") || name === '/api/v1/education/User/checkLogin'){
        if (typeof(body) != "string" && body != null) {
            body = JSON.stringify(body);
        }
        let url = apiConfig.HOST+ name;
        ...
        if (body === "{}"){
            ...
            for (var Key in bodyjson){
                if (typeof bodyjson[Key] === 'string'){
                    bodyjson[Key] = bodyjson[Key]
                }
                formData.append(Key,bodyjson[Key]);
                fromGet += `${Key}=${bodyjson[Key]}&`
            }
            headers.set('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');
            if (type === 'get' || type === 'delete'){
                url = url+`?${fromGet.substring(0,fromGet.length-1)}`;
                requestbody = {
                    method: type,
                    headers
                };
